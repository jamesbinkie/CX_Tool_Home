window.onload = () => {
    const inputs = ["type", "isSymmetric", "W", "H", "sideA", "sideB", "sideC", "bandBottom", "bandRight", "bandLeft"];
    inputs.forEach(id => {
        const el = document.getElementById(id);
        el.addEventListener("change", updateUIAndDraw);
        if (el.tagName === "INPUT") el.addEventListener("input", updateUIAndDraw);
    });
    updateUIAndDraw();
};
...
function getPoints() {
    ...
    // Counter-clockwise order: Bottom-Left, Bottom-Right, Top
    ...
    if (document.getElementById("isSymmetric").checked) {
        ...
        // Using C as base, A as left side, B as right side
        // Points: LeftBase(0, H), RightBase(C, H), Top(C-x, H-y)
...
function drawTriangle(targetCanvas = null) {
    ...
    document.getElementById("safetyWarning").style.display = (angles.some(a => a < 30) && isAnyBanding) ? "block" : "none";
...
function drawDimensions(ctx, pts, scale, offX, offY) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        ctx.save();
        ctx.translate((lx1 + lx2) / 2 + nx * 15, (ly1 + ly2) / 2 + ny * 15);
