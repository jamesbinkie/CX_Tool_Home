...
function validateAndClamp() {
    ...
    // 3. Radius Safety (Sum of adjacent radii < segment length)
    // S1: Bottom (A), S2: Right Edge (D), S3: Inner Horiz (A-C), S4: Inner Vert (B-D), S5: Top Edge (C), S6: Left Edge (B)
    const segments = [tw, lh, (tw - lw), (th - lh), lw, th];
    for (let i = 0; i < 6; i++) {
        ...
        if (i === 3 && r1 < 50) r1 = 50; // Inner Corner Min

        const maxLen = segments[i];
        if (r1 + r2 > maxLen) {
            const factor = maxLen / (r1 + r2 + 0.1);
            r1In.value = Math.floor(r1 * factor);
            r2In.value = Math.floor(r2 * factor);
        }
    }
    // Re-check Corner 4 min after clamping
...
function drawLShape(targetCtx = null) {
    ...
    const pts = getPoints(), tw = parseFloat(document.getElementById("totalW").value), th = parseFloat(document.getElementById("totalH").value);
    ...
    ctx.beginPath(); 
    const startX = corners[0].p[0] * scale + offX, startY = (th - corners[0].p[1]) * scale + offY + corners[0].r; 
