...
function validateAndClamp() {
    ...
    const twIn = document.getElementById("totalW"), thIn = document.getElementById("totalH"), lwIn = document.getElementById("legW"), lhIn = document.getElementById("legH");
    ...
    const r3 = document.getElementById("rad3"); if (parseFloat(r3.value) < 50) r3.value = 50;
    updateUI();
}
...
function drawLShape(targetCtx = null) {
    ...
    const pts = getPoints(), tw = parseFloat(document.getElementById("totalW").value), th = parseFloat(document.getElementById("totalH").value), margin = 140;
    ...
    const corners = pts.map((p, i) => {
        let r = parseFloat(document.getElementById(`rad${i}`).value) || 0; if (i === 3 && r < 50) r = 50;
        const pPrev = pts[(i + 5) % 6], pNext = pts[(i + 1) % 6], len1 = Math.hypot(p[0]-pPrev[0], p[1]-pPrev[1]), len2 = Math.hypot(p[0]-pNext[0], p[1]-pNext[1]);
        r = Math.min(r, len1 * 0.48, len2 * 0.48);
        return { p: p, r: r * scale };
    });
    ctx.beginPath(); const startX = corners[0].p[0] * scale + offX; const startY = (th - corners[0].p[1]) * scale + offY + corners[0].r; ctx.moveTo(startX, startY);
    for (let i = 0; i < 6; i++) {
        const next = corners[(i + 1) % 6], nextNext = corners[(i + 2) % 6];
        ctx.arcTo(next.p[0] * scale + offX, (th - next.p[1]) * scale + offY, nextNext.p[0] * scale + offX, (th - nextNext.p[1]) * scale + offY, next.r);
    }
