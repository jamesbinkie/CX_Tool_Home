    ...
    inputs.forEach(id => {
        ...
        if (el) {
            if (el.tagName === "INPUT" && el.type === "number") {
                // Live preview while typing (NO clamping)
                el.addEventListener("input", () => { refreshMaxLabels(); drawTrapezium(); });
                // Hard safety clamp only when clicking out
...
function validateRadii() {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        let isBanded = false;
        const prevEdge = (i + 3) % 4; // Corner connects prev edge and current edge
        ...
        // 50mm rule only applies if banding flows over THIS corner
...
function updateBandingUI(radii) {
    ...
    // Store old checked state to prevent resetting to fully checked when modifying radii
